...
<nav>
    <div class="nav-container">
        <div style="display:flex; align-items:center;">
            <div class="nav-title">🇪🇸 Verb Master</div>
            <!-- 拉美模式开关 -->
            <div class="toggle-wrapper">
                ...
                <span>Latam (无Vosotros)</span>
            </div>
        </div>

        <ul class="nav-tabs">
            <li class="nav-item active" onclick="setTab('learn', this)">查询</li>
            <li class="nav-item" onclick="setTab('quiz', this)">测试</li>
            <li class="nav-item" onclick="setTab('records', this)">错题</li>
        </ul>
    </div>
</nav>
...
<main>
    ...
    <div id="learn" class="section active">
        <div class="card">
            <span class="group-label">规则动词 (Regulares)</span>
            ...
            <span class="group-label">有规律不规则</span>
            <div class="verb-selector">
            ...
            <span class="group-label">完全不规则</span>
            <div class="verb-selector">
    ...
    <div id="quiz" class="section">
        <div class="card quiz-box">
            <h2 style="border:none; padding:0;">变位特训</h2>
            <div id="q-meta" class="q-meta">加载中...</div>
            <div id="q-verb" class="q-verb">...</div>
            
            <input type="text" id="q-input" class="q-input" placeholder="输入变位..." autocomplete="off">
            <div class="hint">💡 快捷键：输入 ' + a = á, ' + n = ñ</div>
            <div id="q-feedback" class="feedback"></div>
            
            <div>
                <button class="btn-main" onclick="checkAnswer()">提交 (Enter)</button>
                <button style="background:none; border:none; color:#999; cursor:pointer; margin-left:15px;" onclick="nextQuestion()">跳过</button>
            </div>
            
            <div style="margin-top:20px;">
                <button class="char-btn" onclick="addChar('á')">á</button>
                <button class="char-btn" onclick="addChar('é')">é</button>
                <button class="char-btn" onclick="addChar('í')">í</button>
                <button class="char-btn" onclick="addChar('ó')">ó</button>
                <button class="char-btn" onclick="addChar('ú')">ú</button>
                <button class="char-btn" onclick="addChar('ñ')">ñ</button>
            </div>
        </div>
    </div>
    ...
    <div id="records" class="section">
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <h2 style="margin:0; border:none; padding:0;">错题记录</h2>
                <button class="v-btn" onclick="clearRecords()">清空记录</button>
            </div>
...
let currentVerb = 'hablar'; // 记住当前查看的动词
...
const regularEndings = {
    ar: {
        ...
        sub_imp_ra: ['ara','aras','ara','áramos','arais','aran'],
        sub_imp_se: ['ase','ases','ase','ásemos','aseis','asen'],
    ...
    er: {
        ...
        sub_imp_ra: ['iera','ieras','iera','iéramos','ierais','ieran'],
        sub_imp_se: ['iese','ieses','iese','iésemos','ieseis','iesen'],
...
const haberForms = {
    ...
    sub_imp_ra: ['hubiera','hubieras','hubiera','hubiéramos','hubierais','hubieran'],
    sub_imp_se: ['hubiese','hubieses','hubiese','hubiésemos','hubieseis','hubiesen'],
    sub_fut: ['hubiere','hubieres','hubiere','hubiéremos','hubiereis','hubieren']
};

// 3. 动词数据库 (修复：补充缺失的变位、修正不规则动词定义)
const verbsDB = {
    // 规则
    ...
    dormir: { trans: '睡觉', type: 'stem', end: 'ir', rule: 'o-ue', ger: 'durmiendo' },
    poder: { trans: '能够', type: 'stem', end: 'er', rule: 'o-ue', strong_stem: 'pud', future_stem: 'podr', ger_stem: 'pud' },
    jugar: { trans: '玩', type: 'stem', end: 'ar', rule: 'u-ue', pret_yo: 'jugué' },
    pedir: { trans: '请求', type: 'stem', end: 'ir', rule: 'e-i' },
    hacer: { 
        trans: '做', type: 'yoga', end: 'er', yo: 'hago', 
        strong_stem: 'hic', strong_stem_3: 'hiz', future_stem: 'har', 
        part: 'hecho',
        sub_pres: ['haga','hagas','haga','hagamos','hagáis','hagan'] // 修复：补充虚拟式现在时
    },
    conocer: { trans: '认识', type: 'yoga', end: 'er', yo: 'conozco' },
    // 完全不规则
    dar: { 
        trans: '给', type: 'total', end: 'ar', 
        forms: {
            ...
            sub_pres: ['dé','des','dé','demos','deis','den'],
            sub_imp_ra: ['diera','dieras','diera','diéramos','dierais','dieran'],
            sub_imp_se: ['diese','dieses','diese','diésemos','dieseis','diesen'],
            sub_fut: ['diere','dieres','diere','diéremos','diereis','dieren'],
            imp_af: ['-','da','dé','demos','dad','den']
        }
    },
    ser: { 
        trans: '是', type: 'total', end: 'er', 
        forms: {
            pres: ['soy','eres','es','somos','sois','son'],
            imp: ['era','eras','era','éramos','erais','eran'],
            pret: ['fui','fuiste','fue','fuimos','fuisteis','fueron'],
            fut: ['seré','serás','será','seremos','seréis','serán'], // 修复：补充将来时
            cond: ['sería','serías','sería','seríamos','seríais','serían'], // 修复：补充条件式
            sub_pres: ['sea','seas','sea','seamos','seáis','sean'],
            sub_imp_ra: ['fuera','fueras','fuera','fuéramos','fuerais','fueran'],
            sub_imp_se: ['fuese','fueses','fuese','fuésemos','fueseis','fuesen'], // 修复：补充-se形式
            sub_fut: ['fuere','fueres','fuere','fuéremos','fuereis','fueren'], // 修复：补充将来时
            imp_af: ['-','sé','sea','seamos','sed','sean']
        }
    },
    ir: { 
        trans: '去', type: 'total', end: 'ir', ger: 'yendo', part: 'ido', // 修复：补充分词
        forms: {
            pres: ['voy','vas','va','vamos','vais','van'],
            imp: ['iba','ibas','iba','íbamos','ibais','iban'],
            pret: ['fui','fuiste','fue','fuimos','fuisteis','fueron'],
            fut: ['iré','irás','irá','iremos','iréis','irán'], // 修复：补充将来时
            cond: ['iría','irías','iría','iríamos','iríais','irían'], // 修复：补充条件式
    ...
    tener: { 
        trans: '拥有', type: 'yoga', end: 'er', yo: 'tengo', 
        strong_stem: 'tuv', future_stem: 'tendr', imp_af_tu: 'ten',
        sub_pres: ['tenga','tengas','tenga','tengamos','tengáis','tengan'] // 修复：补充虚拟式现在时
    },
    estar: { 
        trans: '在', type: 'yoga', end: 'ar', yo: 'estoy', 
        strong_stem: 'estuv', future_stem: 'estar', // 修复：补充将来时stem
        sub_pres: ['esté','estés','esté','estemos','estéis','estén'] 
    },
    haber: { 
        trans: '有(辅助)', type: 'yoga', end: 'er', yo: 'he', 
        strong_stem: 'hub', future_stem: 'habr', 
        sub_pres: ['haya','hayas','haya','hayamos','hayáis','hayan'] 
    },
    decir: { 
        trans: '说', type: 'yoga', end: 'ir', yo: 'digo', 
        strong_stem: 'dij', future_stem: 'dir', part: 'dicho', 
        imp_af_tu: 'di', rule: 'e-i' 
    },
    venir: { 
        trans: '来', type: 'yoga', end: 'ir', yo: 'vengo', 
        strong_stem: 'vin', future_stem: 'vendr', 
        imp_af_tu: 'ven', rule: 'e-ie' 
    }
};

// 修复：统一时态键名，将sub_imp1改为sub_imp_ra，补充sub_imp_se展示
const layoutStructure = [
    { header: '第一板块：非人称形式', type: 'non_personal' },
    { header: '第二板块：陈述式 (Indicativo)', subsections: [
        { title: '1. 简单时态', tenses: ['pres', 'imp', 'pret', 'fut', 'cond'] },
        { title: '2. 复合时态', tenses: ['comp_pres', 'comp_imp', 'comp_pret', 'comp_fut', 'comp_cond'] }
    ]},
    { header: '第三板块：虚拟式 (Subjuntivo)', subsections: [
        { title: '1. 简单时态', tenses: ['sub_pres', 'sub_imp_ra', 'sub_imp_se', 'sub_fut'] }, // 修复：添加sub_imp_se
        { title: '2. 复合时态', tenses: ['sub_comp_pres', 'sub_comp_imp', 'sub_comp_fut'] }
    ]},
    { header: '第四板块：命令式 (Imperativo)', subsections: [
        { title: '指令', tenses: ['imp_af', 'imp_neg'] }
    ]}
];

const labels = {
    inf: '原形 (Infinitivo)', ger: '副动词 (Gerundio)', part: '分词 (Participio)',
    pres: '现在时', imp: '过去未完成时', pret: '简单过去时', fut: '将来时', cond: '条件式',
    comp_pres: '现在完成时', comp_imp: '过去完成时', comp_pret: '先过去时', comp_fut: '将来完成时', comp_cond: '复合条件式',
    sub_pres: '虚拟式现在时', sub_imp_ra: '虚拟式过去未完成时 (-ra)', sub_imp_se: '虚拟式过去未完成时 (-se)', sub_fut: '虚拟式将来时', // 修复：补充sub_imp_se标签
    sub_comp_pres: '虚拟式现在完成时', sub_comp_imp: '虚拟式过去完成时', sub_comp_fut: '虚拟式将来完成时',
    imp_af: '肯定命令式', imp_neg: '否定命令式'
};
...
function conjugate(verb, tense, pIdx) {
    const vData = verbsDB[verb];
    if (!vData) return '<span class="wrong-msg">未知动词</span>'; // 修复：添加错误处理
    const stem = verb.slice(0, -2);
    ...
    if (tense.includes('comp_')) {
        ...
        if (tense === 'sub_comp_imp') auxKey = 'sub_imp_ra'; // 修复：明确复合虚拟式过去时用-ra形式
        if (tense === 'sub_comp_fut') auxKey = 'sub_fut';
        if (tense === 'comp_cond') auxKey = 'cond';
        const aux = haberForms[auxKey] ? haberForms[auxKey][pIdx] : '错误'; // 修复：添加助动词存在性检查
    ...
    if (tense === 'sub_imp_ra' || tense === 'sub_imp_se' || tense === 'sub_fut') {
        ...
        // 修复：修正词尾重音，添加-se形式的词尾
        let endingMap = {
            'sub_imp_ra': ['ra','ras','ra','áramos','rais','ran'], // 修复：添加重音
            'sub_imp_se': ['se','ses','se','ásemos','seis','sen'], // 修复：添加-se词尾
            'sub_fut': ['re','res','re','áremos','reis','ren']
        };
    ...
    // 修复：命令式否定式 - 保留原虚拟式样式，不提取纯文本
    if (tense === 'imp_neg') {
        if (pIdx === 0) return ''; // Yo无命令式
    ...
    if (!suffix) return '<span class="wrong-msg">无此变位</span>'; // 修复：空值处理
    let currStem = stem;
    let isIrreg = false;
            ...
            block.subsections.forEach(sub => {
                ...
                sub.tenses.forEach(tense => {
                    ...
                    pronouns.forEach((p, idx) => {
                        // 逻辑：如果是拉美模式，且index是4(Vosotros)，则不渲染此行
...
function loadQuestion() {
    ...
    // 修复：测试时态包含sub_imp_ra，移除sub_imp1
...
function checkAnswer() {
    ...
    if(user === correct) {
        fb.innerHTML = '<span class="correct-msg">¡Correcto!</span>';
...
document.getElementById('q-input').addEventListener('input', function(e) {
    ...
    // 检测 ' + vowel 的模式（大小写都支持）
    if (val.endsWith("'a") || val.endsWith("'A")) { e.target.value = val.slice(0, -2) + "á"; }
    else if (val.endsWith("'e") || val.endsWith("'E")) { e.target.value = val.slice(0, -2) + "é"; }
    else if (val.endsWith("'i") || val.endsWith("'I")) { e.target.value = val.slice(0, -2) + "í"; }
    else if (val.endsWith("'o") || val.endsWith("'O")) { e.target.value = val.slice(0, -2) + "ó"; }
    else if (val.endsWith("'u") || val.endsWith("'U")) { e.target.value = val.slice(0, -2) + "ú"; }
    else if (val.endsWith("'n") || val.endsWith("'N")) { e.target.value = val.slice(0, -2) + "ñ"; }
    else if (val.endsWith("u:") || val.endsWith("U:")) { e.target.value = val.slice(0, -2) + "ü"; }
});
